...
const client_ops = @import("client_ops.zig");
...
// --- protocol operations (bodies in `client_ops.zig` since phase 16a) ------
// DECL ALIASES, not forwarders: `cl.walk(...)` resolves exactly as before at
// every call site, with no extra stack frame. What stayed here is the struct,
// its fid/tag bookkeeping and the frame layer they are built on.
pub const version = client_ops.version;
pub const attach = client_ops.attach;
pub const walk = client_ops.walk;
pub const open = client_ops.open;
pub const read = client_ops.read;
pub const write = client_ops.write;
pub const clunk = client_ops.clunk;
pub const stat = client_ops.stat;
pub const CreateResult = client_ops.CreateResult;
pub const create = client_ops.create;
pub const remove = client_ops.remove;
pub const wstat = client_ops.wstat;
pub const flushTag = client_ops.flushTag;
