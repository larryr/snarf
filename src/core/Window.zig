...
// ===========================================================================
// Tag lifecycle (wind.c:437-593) — the composition itself lives in
// `wintag.zig` since phase 16a; these three are the one-line forwarders every
// call site already goes through.
// ===========================================================================

/// `parsetag` (wind.c:437-465) — see `wintag.parseTag`.
pub fn parseTag(w: *Window, a: std.mem.Allocator) error{OutOfMemory}!wintag.Parsed {
    return wintag.parseTag(w, a);
}

/// `winsettag1` (wind.c:497-576) — see `wintag.setTag1`.
pub fn setTag1(w: *Window) Error!void {
    return wintag.setTag1(w);
}

/// `winsettag` (wind.c:577-593) — see `wintag.setTag`.
pub fn setTag(w: *Window) Error!void {
    return wintag.setTag(w);
}
...
/// Shared with `wintag.zig`'s tests (phase 16a) — `pub` for that reason only.
pub const WinHarness = struct {
    fx: Frame.TestFixture,
    chrome: *Chrome,
    body_file: File,
    w: Window,

    pub fn init(seed: []const u8, r: Rect) !*WinHarness {
    ...
    pub fn deinit(h: *WinHarness) void {
        h.w.deinit();
        h.body_file.deinit();
        h.chrome.deinit();
        h.fx.deinit();
...
pub const win_rect = proto.Rect.make(0, 20, 300, 380);
