...
mindmap
  root["/"]
    ...
    n["n"]
      norigin["origin/ — origin server 9P export (if present)"]
        obin["bin/ — also bound into /bin with -a (union)"]
        ofs["fs/ — the server's exported directory"]
        over["version — server identity"]
    bin["bin — union: the origin's bin/, then anything bound later"]
```
